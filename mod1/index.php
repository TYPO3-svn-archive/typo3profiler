...
class tx_typo3profiler_module1 extends t3lib_SCbase
{
	public $pageinfo;
	public $items = array();
	protected $nbElementsPerPage = 20;
	...
	function main()
	{
		...
		$this->content .= '<table width="100%"><tr><td class="functitle" width="50%">' . $LANG->getLL('choose') . '</td><td align="right" width="50%"><input type="button" onclick="jumpToUrl(\'' . t3lib_div::getIndpEnv('TYPO3_REQUEST_DIR') . 'mod.php?M=txtypo3profilerM1_page&flush=true\');" value="' . $GLOBALS['LANG']->getLL('flush') . '"/></td></tr></table>';
	...
	function moduleContent()
	{
		...
		if ($uid !== null) {
			$query['SELECT'] = 'uid,page,parsetime,size,nocache,userint,nbqueries,logts';
			$query['FROM'] = 'tx_typo3profiler_page';
	...
	function formatAResult($row, $table, $title)
	{
		...
		$content .= '<td class="cell" align="center">' . $GLOBALS['LANG']->getLL('parsetime') . '</td>';
		$content .= '<td class="cell" align="center">' . $GLOBALS['LANG']->getLL('size') . '</td>';
		$content .= '<td class="cell" align="center">' . $GLOBALS['LANG']->getLL('nocache') . '</td>';
		$content .= '<td class="cell" align="center">' . $GLOBALS['LANG']->getLL('userint') . '</td>';
		$content .= '<td class="cell" align="center">' . $GLOBALS['LANG']->getLL('queries') . '</td>';
		$content .= '</tr>';
		$content .= '<tr class="db_list_normal">';
		$content .= '<td class="cell">' . $row['page'] . '</td>';
		$page = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows('title', 'pages', 'uid=' . $row['page']);
		$content .= '<td class="cell">' . $page[0]['title'] . '</td>';
		$content .= '<td class="cell" align="center">' . $row['parsetime'] . '</td>';
		$content .= '<td class="cell" align="center">' . $row['size'] . '</td>';
		$content .= '<td class="cell" align="center">' . $row['nocache'] . '</td>';
		$content .= '<td class="cell" align="center">' . $row['userint'] . '</td>';
		$content .= '<td class="cell" align="center">' . $row['nbqueries'] . '</td>';
		$content .= '</tr>';
		$content .= '</table>';
		return $content;
	}
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/typo3profiler/mod1/index.php']) {
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/typo3profiler/mod1/index.php']);
}
// Make instance:
$SOBE = t3lib_div::makeInstance('tx_typo3profiler_module1');
$SOBE->init();
// Include files?
